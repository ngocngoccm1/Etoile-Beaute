(() => {
  ...
  const routeDetails = {
    ...
    'menard-le-mans': {
      eyebrow: 'PRODUITS MENARD DU JAPON', title: 'L’expertise Menard', intro: 'Découvrez les rituels japonais Menard, alliant gestuelle experte et produits d’exception.', cards: [
        { title: 'Hydratation & éclat', text: 'Des formules Menard pour une peau souple, lumineuse et visiblement plus lisse.', items: [['Soin Visage TK', '60 min · 89 €'], ['Éclat Suprême Fairlucent', '70 min · 109 €'], ['Soin Lumière de Saranari', '80 min · 125 €']], image: `${base}assets/menard/saranari.jpg`, imageAlt: 'Gamme Saranari Menard', imageTitle: 'Saranari', imageText: 'Fermeté & anti-âge' },
        { title: 'Anti-âge & rituels', text: 'Des soins haute performance pour les peaux exigeantes et matures.', items: [['Soin Anti-Âge Global Embellir', '90 min · 145 €'], ['Soin des Quatre Saisons Embellir', '90 min · 169 €']], image: `${base}assets/menard/embellir.jpg`, imageAlt: 'Gamme Embellir Menard', imageTitle: 'Embellir', imageText: 'L’anti-âge global' }
      ]
    },
    'massages-le-mans': {
  ...
  const createDetailPage = detail => {
    ...
    if (detail.cards.some(card => card.image)) section.classList.add('eb-detail-page--products');
    section.innerHTML = `<div class="eb-detail-intro"><p>${detail.eyebrow}</p><h1>${detail.title}</h1><span>${detail.intro}</span></div><div class="eb-detail-cards">${detail.cards.map(card => `<article><h2>${card.title}</h2><span>${card.text}</span><ul>${card.items.map(([name, price]) => `<li><b>${name}</b><em>${price}</em></li>`).join('')}</ul>${card.image ? `<figure class="eb-detail-product"><img src="${card.image}" alt="${card.imageAlt}" loading="lazy" width="640" height="400"><figcaption><b>${card.imageTitle}</b><span>${card.imageText}</span></figcaption></figure>` : ''}<a href="${planity}" target="_blank" rel="noopener">Prendre rendez-vous</a></article>`).join('')}</div>`;
    return section;
  };
  const createMenardEditorial = () => {
    ...
    section.querySelectorAll('.eb-menard-text-range').forEach(card => card.remove());
    return section;
  };
  const createMenardProductGallery = () => {
    const section = document.createElement('section');
    section.className = 'eb-menard-product-gallery';
    section.innerHTML = `<header><p>LES ESSENTIELS MENARD</p><h2>Des produits à découvrir au studio</h2><span>Chaque produit peut être conseillé à l’institut pour prolonger le soin à la maison.</span></header><div><article><img src="${base}assets/menard/tk-official.png" alt="Gamme TK Menard"><h3>TK</h3><p>Hydratation et douceur au quotidien.</p></article><article><img src="${base}assets/menard/fairlucent-official.png" alt="Gamme Fairlucent Menard"><h3>Fairlucent</h3><p>Éclat et uniformité du teint.</p></article><article><img src="${base}assets/menard/authent-cream-ii.png" alt="Authent Crème II Menard"><h3>Authent Crème II</h3><p>Un soin d’exception pour revitaliser la peau.</p></article><article><img src="${base}assets/menard/authent-liquid.png" alt="Authent Liquid Menard"><h3>Authent Liquid</h3><p>Une texture précieuse, confortable et sensorielle.</p></article><article><img src="${base}assets/menard/colax.jpg" alt="Colax Menard"><h3>Colax</h3><p>Un geste ciblé pour l’hydratation et la fermeté.</p></article><article><img src="${base}assets/menard/herb-mask.jpg" alt="Herb Mask Menard"><h3>Herb Mask</h3><p>Un masque exfoliant pour raviver l’éclat.</p></article></div><a href="${planity}" target="_blank" rel="noopener">Prendre rendez-vous sur Planity</a></section>`;
    return section;
  };
  ...
  if (window.__ETOILE_ROUTE__ && pageview) {
    ...
    if (detail || targetSection) {
      ...
      if (detail) {
        ...
        if (window.__ETOILE_ROUTE__ === 'menard-le-mans') {
          const menardEditorial = createMenardEditorial();
          detailPage.after(menardEditorial);
          menardEditorial.after(createMenardProductGallery());
        } else {
